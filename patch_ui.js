...
const code = fs.readFileSync('artifacts/cyberplace/src/pages/CompetitionDetailPage.tsx', 'utf8');

const splitPoint = 'const canManageTeam = isAuthenticated && comp.status !== "ended" && isTeamMode;';
if (!code.includes(splitPoint)) {
  console.error("Split point not found!");
  process.exit(1);
}

const [topPart] = code.split(splitPoint);
...
const newCode = topPart + splitPoint + newRender;
fs.writeFileSync('artifacts/cyberplace/src/pages/CompetitionDetailPage.tsx', newCode, 'utf8');
