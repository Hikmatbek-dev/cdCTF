...
export function CdCtfLogo({ className = "h-10 w-auto", size, alt = "cdCTF Logo" }: CdCtfLogoProps) {
  ...
  return (
    <span className={`inline-flex items-center shrink-0 ${className}`} style={style}>
      {/* Light mode: Original logo.png dumaloq */}
      <img
        src="/logo.png"
        alt={alt}
        className="dark:hidden h-full w-auto object-contain rounded-full overflow-hidden"
      />
      {/* Dark mode: Shaffof logo-remove-bg.png */}
      <img
