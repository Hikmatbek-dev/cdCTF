...
async function main() {
  ...
  const res = await client.query("SELECT id, content, content_uz, content_ru FROM lessons;");
  for (const lesson of res.rows) {
    if (!lesson.content || !lesson.content.includes("### In-Depth Analysis")) {
      const newContent = (lesson.content || "") + enrichmentEn;
      const newContentUz = (lesson.content_uz || "") + enrichmentUz;
      const newContentRu = (lesson.content_ru || "") + enrichmentRu;
      
      await client.query(
        "UPDATE lessons SET content = $1, content_uz = $2, content_ru = $3 WHERE id = $4",
        [newContent, newContentUz, newContentRu, lesson.id]
      );
  ...
  await client.end();
  console.log("All lessons enriched successfully!");
}
