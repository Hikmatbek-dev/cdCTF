...
SUITES="${SUITES:-lesson-test-exploit lesson-test-honest auth-sessions roles-permissions two-factor api-tokens oauth passkeys scoring scoreboard competitions profile jobs labs stats streaks reminders referrals seo abuse browser-labs admin-panel admin-content writeups validation body-fields modules-certificates diploma csrf captcha captcha-failclosed rate-limit uz-content}"
FAILED=""
